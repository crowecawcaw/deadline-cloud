...
Jobs can reference files that are stored on shared network storage. The Deadline Client uses a [storage profile]
to determine which paths on the workstation are part of the network storage and do not need to be transferred using job attachments.

[storage profile]: https://docs.aws.amazon.com/deadline-cloud/latest/userguide/storage-profile.html
...
See [MCP Guide](mcp_guide.md) for more information.
