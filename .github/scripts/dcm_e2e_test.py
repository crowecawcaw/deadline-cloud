...
def screenshot(name):
    ...
    if IS_MAC:
        ...
        # Powershell screenshot of all screens
        ps = (
            "Add-Type -AssemblyName System.Windows.Forms,System.Drawing; "
            "$b=[System.Windows.Forms.SystemInformation]::VirtualScreen; "
            "$bmp=New-Object Drawing.Bitmap $b.Width,$b.Height; "
            "$g=[Drawing.Graphics]::FromImage($bmp); "
            f"$g.CopyFromScreen($b.Location,[Drawing.Point]::Empty,$bmp.Size); "
            f"$bmp.Save('{path}');"
        )
    ...
    for app in xa11y.App.list():
        ...
        def walk(el, d=0, maxd=30):
            if d > maxd:
                return
            try:
                print("  " * d + f"{el.role} name={(el.name or '')[:80]!r} value={(el.value or '')[:40]!r}",
                      flush=True)
            except Exception:
                return
            try:
...
def type_into(locator, text):
    """Type text into a field. xa11y.set_value can truncate special chars or fail on
    browser web content; use OS-native keystroke APIs for reliability."""
    locator.focus()
    time.sleep(0.3)
    if IS_LINUX:
        subprocess.run(["xdotool", "type", "--delay", "20", text], check=True)
    elif IS_MAC:
        # osascript keystroke handles all printable chars
...
def click_open_link(br):
    ...
    for label in OPEN_LINK_LABELS:
        try:
            ...
            pass
    # Linux Firefox fallback: auto-redirect page with a 'here' link.
    if IS_LINUX:
        try:
            here = br.locator("link[name='here']").element()
            b = here.bounds
            if b:
                subprocess.run(["xdotool", "mousemove", str(b.x + b.width // 2),
                                str(b.y + b.height // 2), "click", "1"], check=True)
                print(f"xdotool clicked 'here' link at {b.x},{b.y}", flush=True)
                return True
        except Exception:
            pass
    return False
...
def sign_in():
    ...
    for _ in range(6):
        dismissed = False
        for label in ("Start without your data", "Confirm and continue",
                      "Confirm and start browsing", "Skip", "Not now",
                      "Continue without this data", "Continue without Microsoft data"):
            try:
