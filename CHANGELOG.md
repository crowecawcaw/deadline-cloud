...
### Features
* The `deadline job trace-schedule` command now uses batch get APIs for steps and tasks, significantly improving performance for jobs with many tasks. (#1117)
...
### Bug Fixes
* Fixed an issue on Windows where logging in with Deadline Cloud Monitor could corrupt known paths in the `~/.deadline/config` file due to backslash handling differences between .ini libraries. Path separators are now normalized to forward slashes. (#1098)
